...
def home(request):
    ...
    suggestion_users = [user for user in all_profile if user not in my_user]
...
class ShowProfilePageView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['num_posts'] = logged_in_user_posts.count()
        context['button_text'] = 'UnFollow' if FollowersCount.objects.filter(user=page_user).exists() else 'Follow'
        context['user_followers'] = FollowersCount.objects.filter(user=page_user).count()
        context['user_following'] = FollowersCount.objects.filter(follower=page_user).count()
        return context
